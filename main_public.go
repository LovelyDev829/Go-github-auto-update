...
func hasChanges() (bool, string, string, error) {
	...
	lastCommitOnline, err := getonlinecommit.Get(owner, repo)
	if err != nil {
		fmt.Printf("%sError While Online Last Commit  : %s%s\n", Red, err, Reset)
		// fmt.Printf("%sSupperFlag is turned off. you can turn it on at /update-supper-flag.%s\n", Red, Reset)
	}
...
func clone_repo() {
	...
	fmt.Printf("%sRepository cloned successfully to %s%s\n", Yellow, localPath, Reset)
}
...
//-----------------------
func cloneOrOpenRepo(directory, url string) (*git.Repository, error) {
...
func revert(commitSHA string) error {
    ...
    repo, err := cloneOrOpenRepo(localPath, repoURL)
    if err != nil {
